...
from models.embed import DataEmbedding, DataEmbedding_wo_time, DataEmbedding_mTAN
...
class GNNLLM_fsca(nn.Module):
    ...
    def __init__(self, config, data):
        ...
        self.patch_num = (self.num_ref_points - self.patch_size) // self.stride + 1

        # self.padding_patch_layer = nn.ReplicationPad1d((0, self.stride)) 
        self.patch_num += 1
        self.enc_embedding = DataEmbedding_mTAN(d_model=config['d_model'], dropout=config['dropout'],
                                                num_ref_points=config['num_ref_points'], num_encoder_heads=config['num_encoder_heads'], learn_emb=config['learn_emb'],
                                                patch_len=config['patch_size'], stride=config['stride'])
        ...
        self.ln_proj = nn.LayerNorm(configs.d_ff * self.total_len * config["feature_dim"])
        self.out_layer = nn.Linear(configs.d_ff * self.total_len * config["feature_dim"], self.num_classes)

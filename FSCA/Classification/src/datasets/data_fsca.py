...
class Dataset_ECG(Dataset):
    def __init__(self, root_path, data_path=None, flag='train', seq_len=2500, training_flag=1):
        ...
        self.y_data = torch.tensor(self.y_data, dtype=torch.long).unsqueeze(1)
        self.all_ids = torch.arange(self.x_data.size(0), dtype=torch.long)

    def __getitem__(self, index):
        return self.x_data[index], self.y_data[index], self.all_ids[index]

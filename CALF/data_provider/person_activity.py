...
class PersonActivity(object):
	...
	def download(self):
		...
		def save_record(records, record_id, tt, vals, mask, labels):
			# Convert list of timepoints to tensor
			tt = torch.tensor(tt).to(self.device)

            # Stack lists of value, mask, and label tensors into single tensors
			...
			# checks: time, values, mask, labels have same sequence length
            ...
            # Split long sequences into overlapping segments
		...
		for url in self.urls:
            ...
            # Iterate over downloaded text files
			for txtfile in os.listdir(dirname):
				with open(os.path.join(dirname, txtfile)) as f:
					...
					for l in lines:
                        ...
                        # If a new record begins, save the previous one and reset lists
						if cur_record_id != record_id:
							...
							labels = [torch.zeros(len(list(set(self.label_dict.values())))).to(self.device)]
						...
						if time != prev_time:
							...
							labels.append(torch.zeros(len(list(set(self.label_dict.values())))).to(self.device))
							# labels.append(torch.zeros(len(self.label_names)).to(self.device))
							prev_time = time

                        # If this tag is in our list, update value, mask, and count
						if tag_id in self.tag_ids:
                            ...
                            # add label if present and not already set

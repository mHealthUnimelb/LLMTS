...
from Dataset_MM import get_clints_hii_data, get_activity_data, get_physionet_data, get_data
...
def main():
    ...
    if opt.task == 'active':
        ...
        trainloader, validloader, testloader, opt.num_types = get_data(args=opt, dataset=opt.task, device=opt.device, q=opt.quantization, upsampling_batch=False)
        opt.n_classes = 2
        opt.median_len = 72
        # opt.d_model = 32
        # opt.n_head = 1
        ...
        trainloader, validloader, testloader, opt.num_types = get_data(args=opt, dataset=opt.task, device=opt.device,
                                                                       q=opt.quantization, upsampling_batch=False)
        opt.n_classes = 2
        # opt.median_len = 72

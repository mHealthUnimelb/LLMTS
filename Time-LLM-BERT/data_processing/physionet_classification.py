import os
from data_processing.uti import utils as utils
# import uti.utils as utils
import numpy as np
import tarfile
import torch
...
### use quantization -- 0, to keep the original time granularity

# Data format
# type - list
# data[0] - id_number 
# data[][1] - observation timestamp overall, i.e., if we observe one value at each timestamp, then record the timestamp, 48 hours
# data[][2] - observation value, shape = [seq_len, 41]
# data[][3] - observation indicator matrix, value = 0/1 ,0 represents the data is missing, 1 represents the data is observed 
# 41 means 41 variables, 5 general description + 36 time-series related : 
# first 5 are general description

class PhysioNet_classify(object):
	...
	def __init__(self, root, download = False,
		...
		if device == torch.device("cpu"):
			...
			label_a = torch.load(os.path.join(self.processed_folder, self.lb_a), map_location='cpu')
		else:
			data_a = torch.load(os.path.join(self.processed_folder, self.set_a))
			label_a = torch.load(os.path.join(self.processed_folder, self.lb_a))
			...
			for txtfile in os.listdir(dirname):
				...
				with open(os.path.join(dirname, txtfile)) as f:
					...
					for l in lines[1:]:
						...
						# Time in hours
						# check the original time here...
						# print('time:', time)
						time = float(time.split(':')[0]) + float(time.split(':')[1]) / 60.
						# print('after processing:', time)	
				...
				## save the label to a different .pt file?
				labels = None
...
def get_data_min_max(records, device):
	...
	for b, (record_id, tt, vals, mask,labels) in enumerate(records):
		...
		# batch_min = torch.stack([tensor.cuda() for tensor in batch_min])
...
### need to simply modify the collate_fn to make it work for the classification task and imputation task
def patch_variable_time_collate_fn(batch, args, device = torch.device("cpu"), data_type = "train", 
	...
	D = batch[0][2].shape[1]
	# print('check batch:', D) # D=41, they treat the first 5 as general description, and the rest 36 as time-series related...
	combined_tt, inverse_indices = torch.unique(torch.cat([ex[1] for ex in batch]), sorted=True, return_inverse=True) 
	# combined_tt is the timestamps of all the records in this batch, and the inverse_indices is the index of the record_id in the batch
	# ? each participant share the same observed time_points???


	# the number of observed time points , checking how many timepoints in combined_tt are less than args.history, e.g.g '24 hr' .. 
	# history time points....
	n_observed_tp = torch.lt(combined_tt, args.history).sum()
	print('check here:', n_observed_tp) # how many oversed timepoints is smaller than 24 in this batch
	observed_tp = combined_tt[:n_observed_tp] # (n_observed_tp, )
	
	### before put the data into patches, introduce the missingness here to the subsampled data
	...
	for b, (record_id, tt, vals, mask) in enumerate(batch):
		# print('check shape',tt.shape, tt)
		indices = inverse_indices[offset:offset+len(tt)]
		offset += len(tt)
		combined_vals[b, indices] = vals.cuda()
		combined_mask[b, indices] = mask.cuda()
	...
	## just first make the data vector has the same shape... while the irregularity is still there and not only containing the 
	combined_tt = combined_tt[:n_observed_tp].to(device) 
	combined_vals = combined_vals[:, :n_observed_tp].to(device) # (batch_size, n_observed_tp, D)
	combined_mask = combined_mask[:, :n_observed_tp].to(device)

	predicted_tp = pad_sequence(predicted_tp, batch_first=True).to(device) # choose the longest sequnce and then pad the rest??
	...
	# normalization for each batch data...
	...
	# right now, the data is still not patchfied, but seperate into observed data and data to predict
	### get the patch indexes here...
	patch_indices = []
	st, ed = 0, args.patch_size
	for i in range(args.npatch):   ## based on args.npatch to split the observed time points 
		if(i == args.npatch-1):
	...
	# patchify the only the training data?
	data_dict = utils.split_and_patch_batch(data_dict, args, n_observed_tp, patch_indices)


	return data_dict

def imputation_patch_variable_time_collate_fn(batch, args, device = torch.device("cpu"), data_type = "train", 
	data_min = None, data_max = None, time_max = None):
	"""
	Expects a batch of time series data in the form of (record_id, tt, vals, mask) where
		- record_id is a patient id
	...
	# the number of observed time points , checking how many timepoints in combined_tt are less than args.history, e.g.g '24 hr' .. 
	# for imputation task, the args.history = 48
	n_observed_tp = torch.lt(combined_tt, args.history).sum() 
	observed_tp = combined_tt[:n_observed_tp] # (n_observed_tp, )
	
	#### back here to find the patches??? 
	### get the patch indexes here...therefore, each patch is fixed patch-size (xxhours) of data.
	patch_indices = []
	st, ed = 0, args.patch_size
	for i in range(args.npatch):   ## based on args.npatch to split the observed time points 
		if(i == args.npatch-1):
	...
	print('check here:', combined_vals.shape, combined_mask.shape)

	### check here the difference between preprocessing...
	sys.exit()
...
### need to simply modify the collate_fn to make it work for the classification task and imputation task
def patch_variable_time_collate_fn_labelß(batch, args, device = torch.device("cpu"), data_type = "train", 
	data_min = None, data_max = None, time_max = None):
	"""
	Expects a batch of time series data in the form of (record_id, tt, vals, mask) where
		- record_id is a patient id
	...
	## maybe just change the args.history to 48
	n_observed_tp = torch.lt(combined_tt, 48).sum()
	print('check here:', n_observed_tp) # how many oversed timepoints is smaller than 24 in this batch
	observed_tp = combined_tt[:n_observed_tp] # (n_observed_tp, )
	...
	### why we also normalize the timestamp here...
	combined_tt = utils.normalize_masked_tp(combined_tt, att_min = 0, att_max = time_max)
	predicted_tp = utils.normalize_masked_tp(predicted_tp, att_min = 0, att_max = time_max)

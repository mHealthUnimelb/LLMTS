...
def extreme_value(t, kind):
    """
    choose an "extreme" fill value for masking NaNs:

    kind == "min"  ->  *largest* finite value, so it can never be the minimum
    kind == "max"  ->  *smallest* finite value, so it can never be the maximum
    """

    if t.is_floating_point():
        info = torch.finfo(t.dtype)          # carries .min and .max attributes
        return info.max if kind == "min" else info.min
    # int dtype -> use torch.iinfo for limits of integer types
    info = torch.iinfo(t.dtype)
    return info.max if kind == "min" else info.min


def nanmin(x, dim=None, keepdim=False):
    """
    ignore NaNs when taking the minimum

    x        : input tensor
    dim      : dimension to reduce.  None = reduce over *all* elements
    keepdim  : keep the reduced dimension as size-1
    """
    # sentinel that won’t win a min-reduction ( positive infinity for floats, max-int )
    fill_val = extreme_value(x, "min")

    # element-wise replace NaNs by the sentinel
    x_masked = torch.where(
        torch.isnan(x),                              # condition
        torch.tensor(fill_val, dtype=x.dtype, device=x.device),
        x,                                           # original value
    )

    # ordinary min reduction  ── two paths: global vs along a given dim
    if dim is None:
        vals, idx = torch.min(x_masked.view(-1), 0)  # flatten then reduce
        all_nan = torch.isnan(x).all()               # bool scalar
    else:
        vals, idx = torch.min(x_masked, dim=dim, keepdim=keepdim)
        all_nan = torch.isnan(x).all(dim=dim, keepdim=keepdim)  # bool Tensor

    # restore NaN / invalid index (-1) where slice contained *only* NaNs
...
def nanmax(x, dim=None, keepdim=False):
    """
    ignore NaNs when taking the maximum

    x        : input tensor
    dim      : dimension to reduce.  None = reduce over *all* elements
    keepdim  : keep the reduced dimension as size-1
    """
    # sentinel that won’t win a max-reduction (negative infinity for floats, min-int)
    fill_val = extreme_value(x, "max")
...
class Model(nn.Module):
    ...
    def forward(self, x_enc, obs_mask, x_mark_enc, x_dec, x_mark_dec, mask=None):
        if self.task_name == 'long_term_forecast' or self.task_name == 'short_term_forecast':
            dec_out = self.forecast(x_enc, x_mark_enc, x_dec, x_mark_dec)
            return dec_out[:, -self.pred_len:, :]
        elif self.task_name == 'classification':
            print("x_enc shape: ", x_enc.shape)
            class_out = self.classification(x_enc, obs_mask, x_mark_enc, x_dec, x_mark_dec)
            return class_out
        return None
    ...
    def classification(self, x_enc, obs_mask, x_mark_enc, x_dec, x_mark_dec):
        ...
        mask_perm = obs_mask.permute(0, 2, 1).contiguous().reshape(B * N, T, 1)

        x_nan = x_enc.clone()
        x_nan[mask_perm == 0] = float("nan")

        min_values = nanmin(x_nan, dim=1).values.squeeze(-1)     # (B*N)
        max_values = nanmax(x_nan, dim=1).values.squeeze(-1)
        medians = torch.nanmedian(x_nan, dim=1).values.squeeze(-1)

        diff = x_nan[:, 1:, :] - x_nan[:, :-1, :] # (B·N, T-1, 1)
        valid_diff = (mask_perm[:, 1:, :] * mask_perm[:, :-1, :]).bool()
        diff[~valid_diff] = float("nan")
        trends = torch.nansum(diff, dim=1).squeeze(-1) # (B·N)

        lags = self.calcute_lags(x_enc)

        prompt = []
        for b in range(x_enc.shape[0]):
            min_values_str = min_values[b].item()
            max_values_str = max_values[b].item()
            median_values_str = medians[b].item()
            lags_values_str = str(lags[b].tolist())
            prompt_ = (
                f"<|start_prompt|>Dataset description: {self.description}"
                f"Task description: classify the sequence; "
                "Input statistics: "
                f"min value {min_values_str:.4g}, "
                f"max value {max_values_str:.4g}, "
                f"median value {median_values_str:.4g}, "
                f"the trend of input is {'upward' if trends[b] > 0 else 'downward'}, "
                f"top 5 lags are : {lags_values_str}<|<end_prompt>|>"
            )

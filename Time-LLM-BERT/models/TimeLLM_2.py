...
class Model(nn.Module):
    ...
    def classification(self, x_enc, x_mark_enc, x_dec, x_mark_dec, use_reprogramming=True):
        ...
        if use_reprogramming:
            ...
            enc_out = self.project_patch_embedding_layer(enc_out)
            print("enc_out shape: ", enc_out.shape)

        llama_enc_out = torch.cat([prompt_embeddings, enc_out], dim=1)
        print("prompt_embeddings shape: ", prompt_embeddings.shape) # (48, 137, 768)    text embedding   (48, 136, 768)
        print("llama_enc_out shape: ", llama_enc_out.shape) # (48, 449, 768)    time series + text       (48, 448, 768)

        class_out = self.llm_model(inputs_embeds=llama_enc_out).last_hidden_state
        print("class_out shape: ", class_out.shape) # (48, 448, 768)  (batch_size, seq_len, embedding)

        class_out = class_out[:, :, :self.d_ff]
        print("class_out shape: ", class_out.shape) # (48, 448, 128)
        ...
        print("class_out shape: ", class_out.shape) # (24, 2, 128, 448)  (batch_size, num_channels, embedding, seq_len)
        ...
        class_out = class_out.permute(0, 2, 1).contiguous()   # (24, 624, 128)

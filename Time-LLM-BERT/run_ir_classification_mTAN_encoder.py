...
from models import Autoformer, DLinear, TimeLLM, TimeLLM_4, TimeLLM_5, TimeLLM_encoder_CALF, TimeLLM_encoder_mTAN
...
from utils.tools_without_mark_ir_mTAN_encoder import del_files, EarlyStopping, adjust_learning_rate, vali, test, load_content, cal_accuracy, one_hot
...
parser.add_argument('--num_ref_points', type=int, default=128)
parser.add_argument('--learn_emb', default=True, action='store_true')
...
if args.is_training:
    for ii in range(args.itr):
        ...
        elif args.model == 'TimeLLM_encoder_mTAN':
            model = TimeLLM_encoder_mTAN.Model(args, accelerator.device).float()
        else:
        ...
        # model.local_lstm.float()
        # print("after prapare:", model.local_lstm.weight_ih_l0.dtype)
        # model.local_lstm.flatten_parameters()
            ...
            for i, (batch_x, batch_y) in tqdm(enumerate(train_loader)):
                ...
                if args.use_amp:
                    ...
                    if args.output_attention:
                        ...
                        print(model.local_lstm.weight_ih_l0.dtype)
                        # outputs = model(batch_x, observed_mask, x_mark_enc=None, x_dec=None, x_mark_dec=None)
                        outputs = model(torch.cat((observed_data, observed_mask), 2), observed_tp, x_mark_enc=None, x_dec=None, x_mark_dec=None, device=accelerator.device)
                        # outputs = model((torch.cat((observed_data, observed_mask), 2), observed_tp), x_mark_enc=None, x_dec=None, x_mark_dec=None)

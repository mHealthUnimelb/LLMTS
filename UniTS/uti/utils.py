...
def split_and_patch_batch(data_dict, args, n_observed_tp, patch_indices):
	...
	'''
	This is function is used to split the training data into the patches..
	The input time points are already normalized....
	'''
	...
	print("observed_data.shape: ", observed_tp) # all are normalized 


	n_batch, n_tp, n_dim = observed_data.shape # 32, 886, 41
	# prepare the data with shape (batch_size, num_patches, num_timepoints, feature_dim)
	...
	for i in range(args.npatch):
		...
		# Computes how many data points exist in the patch by summing the masks
		# only calculate the existing data points in the patch
		n_data_points = observed_mask[:, st_ind:ed_ind+1].sum(dim=1).max().item()
		max_patch_len = max(max_patch_len, int(n_data_points))
		# çprint("n_data_points: ", n_data_points, "max_patch_len: ", max_patch_len)

	
	
	
	# re-indexing and gathering the data again 
	...
	# can you help check which patches are full of 1s?, but does not mean the variate is full of 1, so each variate still a lot of zeros! 


	split_dict["observed_tp"] = observed_tp_patches # shape is [B, npatch, max_patch_len, D]

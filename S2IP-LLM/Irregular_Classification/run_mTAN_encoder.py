...
parser.add_argument('--draw_heat_map', action='store_true', help='whether to draw a heat map')
...
# model define
parser.add_argument('--num_heads_mtan', type=int, default=1, help='num of heads for mTAN encoder')
parser.add_argument('--learn_emb', action='store_true')
...
if args.is_training:
    ...
    print('>>>>>>>testing : {}<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<'.format(setting))
    if args.task_name == 'ir_forecast':
        mse, mae = exp.test(setting, test=1, args=args)
        print("mse:", mse)
        print("mae:", mae)
    elif args.task_name == 'ir_classification' or args.task_name == 'ir_classification_mTAN':
        if args.data == 'P12' or args.data == 'P19' or args.data == 'eICU' or args.data == 'MIMIC':
            accuracy, auprc, auc = exp.test(setting, test=1, args=args)
            print('accuracy:', accuracy)
            print('auprc:', auprc)
            print('auc:', auc)
        elif args.data == 'PAM' or args.data == 'activity':
            accuracy, auprc, auc, precision, recall, F1 = exp.test(setting, test=1, args=args)
            print('accuracy:', accuracy)
            print('auprc:', auprc)
            print('auc:', auc)

...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        if self.task_name == 'long_term_forecast':
            ...
            encoder_layer = nn.TransformerEncoderLayer(d_model=configs.d_model, nhead=12)
            ...
            # else:
            self.classifier = nn.Linear(int(configs.d_model * (configs.feature_dim + configs.prompt_length)),
                                        self.num_classes)
            ...
            for layer in (self.gpt2, self.linear, self.transformer_encoder, self.classifier):
                layer.cuda()
                layer.train()
    ...
    def classify(self, x_enc, x_mark_enc=None, x_dec=None, x_mark_dec=None):
        ...
        print("x_enc shape: ", x_enc.shape) # (batch, channel, seq_len)

        x = self.linear(x_enc) # (batch, channel, embed)
        x = self.transformer_encoder(x.transpose(0, 1)).transpose(0, 1)
        
        pre_prompted_embedding = x  # map 3(decomp) * patch_size to llm embedding_dim
        ...
        last_embedding = self.gpt2(inputs_embeds=prompted_embedding).last_hidden_state
        print("last_embedding shape: ", last_embedding.shape)  # [batch_size, num_patches + prompt, 768]

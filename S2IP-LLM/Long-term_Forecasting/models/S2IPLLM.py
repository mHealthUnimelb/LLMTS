...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        if self.task_name == 'long_term_forecast':
            ...
            self.in_layer = nn.Linear(configs.patch_size * 3, configs.d_model)
            self.classifier = nn.Linear(configs.d_model, self.num_classes)
    ...
    def classify(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        B, L, M = x_enc.shape
        print(f'B shape: {B.shape}, L shape: {L.shape}, M shape: {M.shape}')
        ...
        decomp_results = np.apply_along_axis(decompose, 1, x.cpu().numpy())
        print("decomp_results shape: ", decomp_results.shape)
        x = torch.tensor(decomp_results).to(self.gpt2.device)
        x = rearrange(x, 'b l c d  -> b c (d l)', c=3)
        ...
        print("x shape: ", x.shape)
        pre_prompted_embedding = self.in_layer(x.float())
        print("pre_prompted_embedding shape: ", pre_prompted_embedding.shape)

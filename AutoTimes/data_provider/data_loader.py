...
class Dataset_ECG(Dataset):
    def __init__(self, root_path='/data/gpfs/projects/punim2341/feixiangz/datasets/ECG/2500', data_path='ECG',
                 flag='train', size=[2500, 0, 0],
                 training_flag=1, token_len=16):
        self.seq_len = size[0]
        self.label_len = size[1]
        self.pred_len = size[2]
        self.token_len = self.seq_len - self.label_len
        ...
        self.data_stamp = torch.load(os.path.join('./dataset', f'{'ECG' + self.data_path}.pt'))
        # Convert to tensors
    ...
    def __getitem__(self, index):
        ...
        # return seq_x, seq_y, seq_x_mark, seq_y_mark
        return self.x_data[index], self.y_data[index], self.data_stamp, self.data_stamp
...
class Dataset_Preprocess_ECG(Dataset):
    def __init__(self, root_path='/data/gpfs/projects/punim2341/feixiangz/datasets/ECG/2500', flag='train', data_path='ECG',
                 size=[2500, 0, 0], training_flag=1, token_len=16):
        self.seq_len = size[0]
        self.label_len = size[1]
        self.pred_len = size[2]
        # self.token_len = self.seq_len - self.label_len
        ...
        self.feature_dim = self.x_data.shape[1]
        print("x_data: ", self.x_data.shape)
        self.data_stamp = np.arange(self.seq_len) / 250
        self.data_stamp = [str(x) for x in self.data_stamp]
        # Convert to tensors
        self.x_data = self.x_data.transpose(0, 2, 1)
        self.x_data = torch.from_numpy(self.x_data)
    ...
    def __getitem__(self, index):
        s_begin = index % self.tot_len
        s_end = s_begin + self.token_len
        # start = self.data_stamp[s_begin]
        # end = self.data_stamp[s_end]
        seq_x_mark = f"This is Time Series from {self.data_stamp[s_begin]} to {self.data_stamp[s_end-1]}"
        print("index", index)
        print("timestamp text:", seq_x_mark)

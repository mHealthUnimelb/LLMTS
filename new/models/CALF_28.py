...
class Model(nn.Module):
    def __init__(self, configs, device):
        ...
        # reconstruction
        self.token_to_var = nn.Linear(configs.d_model, configs.dim)
        self.var_reassembler = nn.Fold(
            output_size = (configs.num_ref_points, 1),
            kernel_size = (self.patch_len, 1),
            stride = (self.stride, 1),
        )
        self.overlap_smoother = nn.Conv1d(
        ...
        for layer in (self.gpt2_text, self.gpt2, self.in_layer, self.out_layer, self.time_proj, self.text_proj, self.token_to_var, self.var_reassembler, self.overlap_smoother):
    ...
    def classification(self, x, time_steps):
        ...
        print("outputs_time shape: ", outputs_time.shape) # (128, 128, 768)
        print("outputs_text shape: ", outputs_text.shape) # (128, 128, 768)
        cls_time = outputs_time.reshape(B, -1)
        cls_text = outputs_text.reshape(B, -1)

        print("outputs_time shape: ", outputs_time.shape) # 128, 98304
        print("outputs_text shape: ", outputs_text.shape) # 128, 98304

        # reconstruction
        # 1) project every patch‑token to original variable space
        var_tok = self.token_to_var(outputs_text)            # [B,N_patches,num_var]
        var_tok = var_tok.permute(0, 2, 1)             # [B,num_var,N_patches]
        # 2) inverse‑patch: Fold expects (B,d_in*N_patches,1)
        recon = self.var_reassembler(var_tok.unsqueeze(-1)).squeeze(-1)  # [B,in_dim,T]
        recon = self.overlap_smoother(recon)           # smooth overlap
        recon = recon.permute(0, 2, 1).contiguous()    # [B,T,in_dim] same as observed_data

        outputs_time = self.out_layer(outputs_time)
        outputs_text = self.out_layer(outputs_text)

        return {
            'outputs_text': cls_text,
            'outputs_time': cls_time,
            'intermidiate_time': intermidiate_feat_time,
            'intermidiate_text': intermidiate_feat_text,

...
class enc_mtan(nn.Module):
    ...
    def forward(self, x, time_steps):
        ...
        # convert shape into [batch_size * num_patches, patch_len, dim]
        out = rearrange(out_patches, 'b n l d -> (b n) l d')  # [batch_size * num_patches, patch_len, dim]
...
class Model(nn.Module):
    def __init__(self, configs, device):
        ...
        if self.task_name == 'long_term_forecast' or self.task_name == 'short_term_forecast':
            ...
            self.out_layer = nn.Linear(configs.d_model * self.num_patches * configs.patch_len, configs.num_class)
            # print("configs.d_model * configs.enc_in: ", configs.d_model * configs.enc_in) # 768 * 7 = 5376
            # encoder-only classification head
            self.encoder_only_head = nn.Linear(configs.d_model * self.num_patches * configs.patch_len, configs.num_class)

...
class Encoder_PCA(nn.Module):
    def __init__(self, input_dim, word_embedding, hidden_dim=4096, num_heads=1, num_encoder_layers=1, device='cpu',
...
class Model(nn.Module):
    def __init__(self, configs, device):
        ...
        self.llama_config.num_hidden_layers = configs.gpt_layers
        ...
        print("llama", self.llama)

        self.tokenizer = LlamaTokenizer.from_pretrained('huggyllama/llama-7b')

        self.word_embeddings = self.llama_text.embed_tokens.weight.to(device)
        ...
        for i, (name, param) in enumerate(self.llama.named_parameters()):
            # if 'input_layernorm' in name or 'post_attention_layernorm' in name or 'lora' in name:
            if 'norm' in name or 'rotary_emb' in name or 'lora' in name:
                param.requires_grad = True
            else:
                param.requires_grad = False

        for i, (name, param) in enumerate(self.llama_text.named_parameters()):
        ...
        inputs = self.tokenizer(prompt_text, return_tensors='pt')
        input_ids = inputs["input_ids"]
        self.prompt_embeddings = self.llama_text.embed_tokens(input_ids).to(device)

...
# patching before encoder (time-synchronous)
import torch
...
class enc_mtan(nn.Module):
    ...
    def forward(self, x, time_steps):
        ...
        x_patches = x.unfold(dimension=1, size=self.patch_len, step=self.stride) # Shape: [batch_size, num_patches, dim, patch_len]
        ...
        if self.learn_emb:
            ...
            query = self.learn_time_embedding(self.query.unsqueeze(0)).to(self.device)
            # query = self.learn_time_embedding(query_patches).to(self.device)
        ...
        # key += patch_pos_emb_key

        out = self.time_att(query, key, x_patches, mask)  # batch_size, num_ref_points, embed_dim
...
class enc_mtan_activity(nn.Module):
    def __init__(self, input_dim, nhidden=16,
                 embed_time=16, num_heads=1, learn_emb=True, freq=10., patch_len=16, stride=8, device='cuda'):
        super(enc_mtan_activity, self).__init__()
        assert embed_time % num_heads == 0
        self.freq = freq
        self.embed_time = embed_time
        self.learn_emb = learn_emb
    ...
    def forward(self, x, time_steps):
        ...
        out = self.time_att(key, key, x, mask)  # batch_size, num_ref_points, embed_dim
        print("out shape: ", out.shape)
        out = out.permute(1, 0, 2)  # Shape: [num_ref_points, batch_size, nhidden]
        out, _ = self.var_att(out, out, out)
        out = out.permute(1, 0, 2)
...
class Encoder_PCA(nn.Module):
    def __init__(self, input_dim, word_embedding, hidden_dim=768, num_heads=1, num_encoder_layers=1, device='cpu',
                 num_ref_points=128, latent_dim=32, learn_emb=True, patch_len=16, stride=8, num_ca_heads=1, prompt_embeddings=None, configs=None):
        ...
        if configs.data == 'activity':
            self.encoder = enc_mtan_activity(input_dim, nhidden=hidden_dim, embed_time=128, num_heads=num_ca_heads, learn_emb=learn_emb, patch_len=patch_len,stride=stride).to(device)
        else:
...
class Model(nn.Module):
    def __init__(self, configs, device):
        ...
        self.in_layer = Encoder_PCA(configs.dim, self.word_embeddings, hidden_dim=configs.d_model,
                                    ...
                                    num_ca_heads=configs.num_ca_heads, prompt_embeddings=self.prompt_embeddings, configs=configs)

        # self.attention_pooling = AttentionPooling(configs.d_model)
        # self.gru = nn.GRU(configs.d_model, configs.d_model)
        # self.lstm = nn.LSTM(input_size=configs.d_model, hidden_size=configs.d_model, batch_first=True)
        ...
        if self.task_name == 'long_term_forecast' or self.task_name == 'short_term_forecast':
            ...
            self.num_patches = math.ceil((2881 - configs.patch_len) / configs.stride) + 1
    ...
    def classification(self, x, time_steps):
        ...
        outputs_time = outputs_time1.reshape(B, -1)
        outputs_text = outputs_text1.reshape(B, -1)
        print("outputs_time shape: ", outputs_time.shape) # (128, 128, 768)
        print("outputs_text shape: ", outputs_text.shape) # (128, 128, 768)
        ...
        # _, (outputs_time, _) = self.lstm(outputs_time)
        # _, (outputs_text, _) = self.lstm(outputs_text)

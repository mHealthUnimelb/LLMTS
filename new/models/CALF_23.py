...
class Encoder_PCA(nn.Module):
    def __init__(self, input_dim, word_embedding, hidden_dim=768, num_heads=1, num_encoder_layers=1, device='cpu',
                 ...
                 prompt_embeddings=None, top_k=300):
        ...
        embed_dim = self.word_embeddings.size(1)
        # build FAISS index
        w_emb = F.normalize(self.word_embeddings, p=2, dim=1)
        we_np = w_emb.detach().cpu().numpy().astype(np.float32)
        self.index = faiss.IndexHNSWFlat(embed_dim, 32, faiss.METRIC_INNER_PRODUCT)
        # Adjust search parameters (bigger efSearch => more accurate but slower)
        self.index.hnsw.efConstruction = 40
        self.index.hnsw.efSearch = 64
        self.index.verbose = False

        # Add all vocab embeddings to the FAISS index
        self.index.add(we_np)
        print("FAISS index built for vocabulary embeddings.")
    ...
    def reduce_vocab(self, query_emb, top_k=600):
        # 1) Move query to CPU float32
        query_np = F.normalize((query_emb.detach().cpu()), p=2, dim=1).numpy().astype(np.float32)

        # 2) Search in FAISS index.
        #    distances: shape [Q, top_k]
        #    indices:   shape [Q, top_k]
        distances, indices = self.index.search(query_np, top_k)

        # 3) Flatten or pick unique token IDs among all queries
        #    If you want unique tokens across the entire batch:
        # chosen_token_ids = np.unique(indices.flatten())
        flat_dists = distances.reshape(-1)  # shape = [Q * top_k]
        flat_ids = indices.reshape(-1)  # shape = [Q * top_k]
        # Sort by distance descending
        sorted_idxs = np.argsort(-flat_dists)  # shape = [Q*top_k], from largest to smallest

        # Pick top-K unique IDs
        chosen_ids = []
        chosen_sims = []
        seen = set()

        for idx in sorted_idxs:
            token_id = flat_ids[idx]
            sim = flat_dists[idx]
        ...
        print("similarity", chosen_sims)
        return chosen_ids  # shape [top_k]
    ...
    def forward(self, x, time_steps):
        ...
        chosen_token_ids = self.reduce_vocab(query_emb=query_x, top_k=self.top_k)
        ...
        token_list = self.tokenizer.convert_ids_to_tokens(chosen_token_ids)

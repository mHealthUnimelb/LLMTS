...
class Encoder_PCA(nn.Module):
    def __init__(self, input_dim, word_embedding, hidden_dim=768, num_heads=1, num_encoder_layers=1, device='cpu',
        ...
        self.cross_attention_2 = nn.MultiheadAttention(embed_dim=hidden_dim, num_heads=num_ca_heads)
    ...
    def forward(self, x, time_steps):
        ...
        final, w2_ = self.cross_attention_2(x_time, x, x)

        # combined = torch.stack([x, x_time], dim=2)
        # combined = combined.view(B, x_time.shape[1]*2, x_time.shape[2])
...
class Model(nn.Module):
    def __init__(self, configs, device):
        ...
        self.word_embeddings = self.gpt2.wte.state_dict()['weight'].to(device)
        ...
        self.prompt_embeddings = self.gpt2.wte(input_ids).to(device)
    ...
    def classification(self, x, time_steps):
        ...
        outputs_time1 = self.in_layer(x, time_steps)

...
# multi-scale multiple num_ref_points
# after cross-attention, multi reference points
import torch
...
class multiTimeAttention(nn.Module):
    ...
    def __init__(self, input_dim, nhidden=16,
        ...
        print("embed_time", self.embed_time)
...
class enc_mtan(nn.Module):
    ...
    def forward(self, x, time_steps, query):
        ...
        if self.learn_emb:
            key = self.learn_time_embedding(time_steps).to(self.device)
            query = self.learn_time_embedding(query.unsqueeze(0)).to(self.device)
        else:
            key = self.time_embedding(time_steps, self.embed_time).to(self.device)
            query = self.time_embedding(query.unsqueeze(0), self.embed_time).to(self.device)
        ...
        # lstm
        out = out_patches.view(batch_size * num_patches, self.patch_len, self.nhidden)
        _, (out, _) = self.local_lstm(out)
        out = out.squeeze(0).view(batch_size, num_patches, self.nhidden)
        print("out shape: ", out.shape)
...
class Encoder_PCA(nn.Module):
    def __init__(self, input_dim, word_embedding, hidden_dim=768, num_heads=1, num_encoder_layers=1, device='cpu',
                 ref_points_scales=(64, 128, 256, 512), latent_dim=32, learn_emb=True, patch_len=16, stride=8,
                 num_ca_heads=1, prompt_embeddings=None):
        ...
        self.ref_scales = tuple(ref_points_scales)
        # self.encoder = enc_mtan_rnn(input_dim, torch.linspace(0, 1., num_ref_points), latent_dim,
        #                             nhidden=hidden_dim,
        #                             embed_time=128, learn_emb=learn_emb, num_heads=num_heads).to(device)
        self.encoder = enc_mtan(input_dim=input_dim, nhidden=hidden_dim,
    ...
    def forward(self, x, time_steps):
        ...
        for n_ref in self.ref_scales:
            query = torch.linspace(0., 1., n_ref, device=x.device)
            # x = self.linear(x)

            # x = self.transformer_encoder(x.transpose(0, 1)).transpose(0, 1)
            # x = self.encoder(x.transpose(0, 1), time_steps).transpose(0, 1)

            time_out = self.encoder(x, time_steps, query)
            print("x shape after encoder", x.shape)

            x_time = time_out

            q = time_out.transpose(0, 1)
            k = v = word_embedding.transpose(0, 1)
            text_out, w_ = self.cross_attention(q, k, v)
            print("weights shape: ", w_.shape)

            text_out = text_out.transpose(0, 1)

            text_scales_out.append(text_out)
            time_scales_out.append(x_time)

        # max_patches = max(t.shape[1] for t in time_scales_out)
        # max_len = max(t.shape[1] for t in text_scales_out)

        # padded_time_branch_out = [F.pad(t, (0, 0, 0, max_patches - t.shape[1])) for t in time_scales_out]
        # padded_text_branch_out = [F.pad(t, (0, 0, 0, max_len - t.shape[1])) for t in text_scales_out]
        ...
        padded_time_branch_out = torch.cat(time_scales_out, dim=1)
        padded_text_branch_out = torch.cat(text_scales_out, dim=1)
...
class Model(nn.Module):
    def __init__(self, configs, device):
        ...
        self.in_layer = Encoder_PCA(input_dim=configs.dim,
                                    word_embedding=self.word_embeddings,
                                    hidden_dim=configs.d_model,
                                    num_heads=configs.num_ca_heads,
                                    device=device,
                                    ref_points_scales=(64, 128, 256, 512),
        ...
        if self.task_name == 'long_term_forecast' or self.task_name == 'short_term_forecast':
            ...
            self.num_patches = sum(
                math.ceil((n_ref - configs.patch_len) / configs.stride) + 1 for n_ref in
                self.in_layer.ref_scales)

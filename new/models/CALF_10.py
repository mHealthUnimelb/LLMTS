...
# add prompt before cross match module and concate with time token, like [prompt, time token]
import torch
...
class Encoder_PCA(nn.Module):
    ...
    def forward(self, x, time_steps):
        ...
        x = torch.cat([batch_prompt_embeddings, x], dim=1)
        print("time token shape: ", x.shape)

...
class Dual(nn.Module):
    def __init__(
        ...
        if self.task == 'forecast':
            ...
            self.classifier = nn.Linear(self.num_nodes * self.channel, num_classes)
    ...
    def forecast(self, input_data, input_data_mark, embeddings):
    ...
    def classify(self, input_data, embeddings):
        B = input_data.size(0)
    ...
    def classify(self, input_data, input_data_mark=None, embeddings=None):
        if self.task == 'forecast':
            output = self.forecast(input_data, input_data_mark, embeddings)
        elif self.task == 'classification':
            output = self.classify(input_data, embeddings)

...
from data_provider.data_loader_save import Dataset_ETT_hour, Dataset_ETT_minute, Dataset_Custom, Dataset_ECG
...
def parse_args():
    ...
    parser.add_argument("--training_flag", type=int, default=1)
    parser.add_argument("--seq_len", type=int, default="2500")
...
def get_dataset(data_path, flag, input_len, output_len, args=None):
    datasets = {
    ...
    if args.data_name == 'ECG':
        return dataset_class(root_path=args.root_path, flag=flag, data_path=args.data_path, seq_len=args.input_len)
    return dataset_class(flag=flag, size=[input_len, 0, output_len], data_path=data_path)

def save_embeddings(args):
    device = torch.device(args.device if torch.cuda.is_available() else "cpu")
    train_set = get_dataset(args.data_path, 'train', args.input_len, args.output_len, args)
    test_set = get_dataset(args.data_path, 'test', args.input_len, args.output_len, args)
    val_set = get_dataset(args.data_path, 'val', args.input_len, args.output_len, args)

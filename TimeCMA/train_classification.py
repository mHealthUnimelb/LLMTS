...
from data_provider.data_loader_emb import Dataset_ETT_hour, Dataset_ETT_minute, Dataset_Custom, Dataset_ECG
...
def parse_args():
    ...
    parser.add_argument("--task", type=str, default="forecast", help="forecast or classification")
...
class trainer:
    def __init__(
        ...
        self.model = Dual(
            ...
            dropout_n=dropout_n, d_llm=d_llm, e_layer=e_layer, d_layer=d_layer, head=head, task=task, num_classes=num_classes
        )
    ...
    def train(self, input, embeddings, true_labels):
        self.model.train()
        self.optimizer.zero_grad()
        logits = self.model(input, embeddings)
        loss = self.loss(logits, true_labels)
        loss.backward()
        if self.clip is not None:
            torch.nn.utils.clip_grad_norm_(self.model.parameters(), self.clip)
        self.optimizer.step()
        train_probs = torch.nn.functional.softmax(logits)
        train_predictions = torch.argmax(train_probs, dim=1)
        return loss.item(), mae.item()
    
    def eval(self, input, embeddings, true_labels):
        self.model.eval()
        with torch.no_grad():
            logits = self.model(input, embeddings)
        loss = self.loss(logits, true_labels)

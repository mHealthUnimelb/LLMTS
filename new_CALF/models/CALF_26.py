...
# before cross-attention, concate all scales' time series representation and then do cross-attention
import torch
...
class Encoder_PCA(nn.Module):
    def __init__(self, input_dim, word_embedding, hidden_dim=768, num_heads=1, num_encoder_layers=1, device='cpu',
                 ref_points_scales=(128, 256, 512), latent_dim=32, learn_emb=True, patch_len=16, stride=8,
    ...
    def forward(self, x, time_steps):
        ...
        # add prompt for word token
        # word_embedding = torch.cat([self.prompt_embeddings.squeeze(0), self.word_embedding], dim=0)
        ...
        for n_ref in self.ref_scales:
            ...
            print("x shape after encoder", time_out.shape)
            time_scales_out.append(time_out)

        x_time = torch.cat(time_scales_out, dim=1)

...
class multiTimeAttention(nn.Module):
    ...
    def attention(self, query, key, value, mask=None, dropout=None):
        ...
        p_attn_time = F.softmax(scores, dim=-2)
        if dropout is not None:
            p_attn_time = dropout(p_attn_time)
        attn_out_time = torch.sum(p_attn_time * value.unsqueeze(-3), -2) # (batch_size, num_heads, seq_len, num_variables)

        # computer attention over variables
        query_var = attn_out_time
        key_var = attn_out_time
        value_var = attn_out_time

        scores_var = torch.matmul(query_var, key_var.transpose(-2, -1)) / math.sqrt(dim)
        scores = scores.unsqueeze(-1).repeat_interleave(dim, dim=-1)
        print("scores shape: ", scores.shape)
        print("mask shape: ", mask.shape)
        if mask is not None:
            scores = scores.masked_fill(mask.unsqueeze(-3) == 0, -1e9)
        p_attn_var = F.softmax(scores, dim=-1)
        if dropout is not None:
            p_attn_var = dropout(p_attn_var)

        attn_out = torch.matmul(p_attn_var, value_var)
        attn_out = attn_out.sum(dim=3)

        return attn_out, (p_attn_time, p_attn_var)

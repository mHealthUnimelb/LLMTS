...
class Model(nn.Module):
    def __init__(self, configs, device):
        ...
        self.reconstruct_time = nn.Linear(configs.d_model * self.num_patches, configs.seq_len * configs.enc_in)
    ...
    def classification(self, x, time_steps):
        ...
        logits_time = self.out_layer(outputs_time)
        logits_text = self.out_layer(outputs_text)

        # reconstruct time series
        time_recon = self.reconstruct_time(outputs_text)
        time_recon = time_recon.view(B, -1, M)

        return {
            'outputs_text': logits_text,
            'outputs_time': logits_time,
            'intermidiate_time': intermidiate_feat_time,
            'intermidiate_text': intermidiate_feat_text,
            'encoder_only': encoder_only_logits,
            'time_recon': time_recon
        }

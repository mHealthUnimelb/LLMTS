...
class VariableEmbedding(nn.Module):
    def __init__(self, num_variables, embedding_dim=16):
        super().__init__()
        self.proj_in = nn.Linear(1, embedding_dim)  # project raw scalar to embedding_dim
        self.emb = nn.Embedding(num_variables, embedding_dim)

    def forward(self, features):
        batch, seq_len, num_vars = features.shape

        # project raw feature values to a vector (batch, seq_len, num_vars, d_emb)
        feat_proj = self.proj_in(features.unsqueeze(-1))

        # create indices for each variable
        var_indices = torch.arange(num_vars, device=features.device).unsqueeze(0).unsqueeze(0).expand(batch, seq_len,
                                                                                                      num_vars)

        # Lookup learned variable embeddings
        var_emb = self.emb(var_indices)  # (batch, seq_len, num_vars, embedding)
        print("var emb shape: ", var_emb.shape) # (64, 216, 41, 768)

        # Combine the projected feature and learned embedding (element-wise addition)
        out = feat_proj + var_emb  # shape: (batch, seq_len, num_vars, d_emb)
        print("var_emb shape: ", out.shape) # (64, 216, 41, 768)
...
class multiTimeAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None, dropout=None):
        ...
        print(f"batch: {batch}, seq_len: {seq_len}, dim: {dim}")  # 64, 216, 62976
        if mask is not None:
...
class enc_mtan(nn.Module):
    ...
    def __init__(self, input_dim, query, nhidden=16,
        ...
        """
        input_dim equals the dimension of the first half of x
        feed x as a concatenation [features_embedding, mask_vectorized], where both
        halves have dimension: num_vars * embedding_dim
        """
        ...
        self.embed_time = embed_time
        self.learn_emb = learn_emb
        self.dim = input_dim  # input_dim = 2 * num_vars * embedding 82 * 768 = 62976
        self.device = device
    ...
    def forward(self, x, time_steps):
        # x shape: (batch, seq_len, 2*self.dim)
        time_steps = time_steps.cpu()
        mask = x[:, :, self.dim:]
        mask = torch.cat((mask, mask), 2)
        if self.learn_emb:
...
class Encoder_PCA(nn.Module):
    def __init__(self, input_dim, word_embedding, hidden_dim=768, num_heads=1, num_encoder_layers=1, device='cpu',
        ...
        self.emb_dim = 16
        self.new_feature_dim = self.num_vars * self.emb_dim  # dimension after variable embedding

        # initialize variable embedding module
        self.variable_embedding = VariableEmbedding(num_variables=self.num_vars, embedding_dim=self.emb_dim)

        # project from variable embedding dimension to the time attention dimension
        # self.variable_embedding_project = nn.Linear(self.new_feature_dim, self.emb_dim)
        ...
        self.encoder = enc_mtan(self.new_feature_dim, torch.linspace(0, 1., num_ref_points), nhidden=hidden_dim,
                                embed_time=128, num_heads=num_heads, learn_emb=learn_emb, patch_len=patch_len,
                                stride=stride).to(device)
    ...
    def forward(self, x, time_steps):
        ...
        # split into raw features and mask
        features = x[:, :, :self.num_vars]  # (batch, seq_len, num_vars)
        mask = x[:, :, self.num_vars:]  # (batch, seq_len, num_vars)

        var_embedding = self.variable_embedding(features)  # (batch, seq_len, num_vars, embedding_dim) (64, 216, 41, 768)

        # flatten the embedded features: (B, seq_len, num_vars * embedding_dim)
        flattened_var_embedding = var_embedding.view(B, -1, self.num_vars * self.emb_dim)
        print("flattened_features shape: ", flattened_var_embedding.shape) # (64, 216, 31488)

        # transform (B, seq_len, num_vars * embedding_dim) to (B, seq_len, embedding_dim)
        # transformed_var_embedding = self.variable_embedding_project(flattened_var_embedding)

        print("mask shape: ", mask.shape)
        # vectorize the mask by replicating each mask entry embedding_dim times
        mask_emb = mask.unsqueeze(-1).expand(-1, -1, -1, self.emb_dim).contiguous().view(B, -1,
                                                                                         self.num_vars * self.emb_dim)
        print("mask_emb shape: ", mask_emb.shape) # (64, 216, 31488)

        # concatenate the feature embedding and vectorized mask along the last dimension
        x = torch.cat([flattened_var_embedding, mask_emb], dim=-1)
        print("x shape after concatenating: ", x.shape) # (64, 216, 62976)

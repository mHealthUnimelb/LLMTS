...
class Patching(nn.Module):
    ...
    def forward(self, x):
        ...
        # num_patches = (L - P) // S + 1
        num_patches = math.ceil((L - P) / S) + 1
        total_length = (num_patches - 1) * S + P

        # pad the input sequence if necessary
        if total_length > L:
            pad_length = total_length - L
            pad_layer = nn.ReplicationPad1d((0, pad_length))
            x = pad_layer(x)

        # use unfold to create patches
        x_patched = x.unfold(dimension=-1, size=P, step=S)  # (B, M, num_patches, P)
        print("x_patched shape: ", x_patched.shape)
        x_patched = x_patched.permute(0, 2, 1, 3).contiguous()  # (B, num_patches, M, P)
        x_patched = x_patched.view(B, num_patches, M * P)  # (B, num_patches, P*M)
        return x_patched
...
class Model(nn.Module):
    def __init__(self, configs, device):
        ...
        input_dim = self.patch_len * configs.enc_in
        num_patches = math.ceil((configs.seq_len - self.patch_len) / self.stride) + 1
        # self.in_layer = Encoder_PCA(configs.seq_len, word_embedding, hidden_dim=configs.d_model)
        self.in_layer = Encoder_PCA(input_dim, word_embedding, hidden_dim=configs.d_model)

        if self.task_name == 'long_term_forecast' or self.task_name == 'short_term_forecast':
            self.out_layer = nn.Linear(configs.d_model, configs.pred_len)
        elif self.task_name == 'classification':
            self.out_layer = nn.Linear(configs.d_model * num_patches, configs.num_class)
        elif self.task_name == 'imputation':
            self.out_layer = nn.Linear(configs.d_model, configs.seq_len)
        elif self.task_name == 'anomaly_detection':
            self.out_layer = nn.Linear(configs.d_model, configs.seq_len)

        for layer in (self.gpt2_text, self.gpt2, self.patching, self.in_layer, self.out_layer, self.time_proj, self.text_proj):
    ...
    def classification(self, x):
        ...
        outputs_time1, outputs_text1 = self.in_layer(x_patched)

...
from utils.tools_without_mark import del_files, EarlyStopping, adjust_learning_rate, vali, load_content
...
for ii in range(args.itr):
    ...
    for epoch in range(args.train_epochs):
        ...
        vali_loss, vali_accuracy, vali_auprc = vali(args, accelerator, model, vali_data, vali_loader, criterion, metric)
        vali_losses.append(vali_loss)
        vali_accuracies.append(vali_accuracy)
        vali_auprcs.append(vali_auprc)
        test_loss, test_accuracy, test_auprc = vali(args, accelerator, model, test_data, test_loader, criterion, metric)
        ...
        accelerator.print(
            "Epoch: {0} | Train Loss: {1:.7f} Train Acc: {2:.7f} Train AUPRC: {3:7f} Vali Loss: {4:.7f} Vali Acc: {5:.7f} Vali AUPRC: {6:7f} Test Loss: {7:.7f} Test Acc: {8:.7f} Test AUPRC: {9:.7f}".format(
                epoch + 1, train_loss, train_accuracy, train_auprc, vali_loss, vali_accuracy, vali_auprc, test_loss, test_accuracy, test_auprc))
